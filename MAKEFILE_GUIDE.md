...
このドキュメントは、プロジェクトの Makefile の構造、依存関係、使用方法を説明します。
...
各コンポーネントのランタイムでの役割と依存関係を理解することが重要です:
...
`make uninstall` で **Colima → Buildx → Docker** の順に削除するのは:
...
逆順でインストールするのは論理的な流れのため（CLI → プラグイン → ランタイム）。

### クリーンアップフロー (`make uninstall`)
